...
<template>
  <div>
    <h3 class="text-h6 mb-6 text-neutral-900 dark:text-neutral-100">{{ heading }}</h3>
    <div class="flex flex-wrap gap-4">
      <div v-for="step in steps" :key="step.key" class="w-36">
        <div
        ...
        />
        <div class="mt-2">
          <p class="text-paragraph-small-500 text-neutral-900 dark:text-neutral-100">{{ step.key }}</p>
          <button
            type="button"
            class="block font-mono text-paragraph-xsmall text-neutral-500 transition-colors hover:text-primary-500 dark:text-neutral-400 dark:hover:text-primary-300"
            :title="`Copy ${step.hex}`"
